...
//#region node_modules/.nitro/vite/services/ssr/assets/store-CN_mVyeU.js
function cn(...inputs) {
...
function fulfillInitialSuccess(world, obligation) {
	if (world.bookings.some((booking) => booking.requestId === obligation.requestId)) throw new DomainError("INVALID_TRANSITION", "Booking already exists");
	const request = requireRequest(world, obligation.requestId);
	const hold = activeHoldFor(world, request.id);
	if (!hold) throw new DomainError("HOLD_EXPIRED", "Hết thời gian giữ phòng — cần xử lý hoàn tiền");
...
function resolveUnknown(world, input) {
	...
	if (obligation.kind !== "INITIAL") return {
		world,
		attempt
	};
	const fulfilled = fulfillInitialSuccess(world, obligation);
...
var useBookingStore = create()(persist((set, get) => ({
	...
	hostAccept: (requestId) => {
		set({ world: acceptRequest(get().world, {
			requestId,
			actor: { persona: "HOST" }
		}).world });
	},
	hostRecordPayment: (obligationId, outcome) => {
		set({ world: recordPayment(get().world, {
			obligationId,
			outcome,
			actor: { persona: "HOST" }
		}).world });
	...
	hostResolveUnknown: (attemptId, outcome) => {
		set({ world: resolveUnknown(get().world, {
			attemptId,
			outcome,
			actor: { persona: "HOST" }
		}).world });
	...
	butlerCheckIn: (stayId) => {
		set({ world: checkInStay(get().world, {
			stayId,
			actor: {
				persona: "BUTLER",
	...
	butlerCheckOut: (stayId) => {
		set({ world: checkOutStay(get().world, {
			stayId,
			actor: {
				persona: "BUTLER",
				butlerId: BUTLER_LINH
	...
	butlerNoShow: (stayId, reason) => {
		set({ world: markDidNotOccur(get().world, {
			stayId,
			actor: {
				persona: "BUTLER",
				butlerId: BUTLER_LINH
	...
	butlerIncident: (stayId, note, hasPhoto) => {
		set({ world: reportIncident(get().world, {
			stayId,
			actor: {
				persona: "BUTLER",
				butlerId: BUTLER_LINH
...
//#endregion
export { stayGuestLabel as C, paymentPlanLabel as S, villas as T, isIsoDate as _, SALE_MAI as a, opsLists as b, bookabilityCopy as c, formatDateRange as d, formatLongDate as f, isAvailable as g, guestLabel as h, DomainError as i, cn as l, getVilla as m, BUTLER_LINH as n, bedroomLabel as o, formatVnd as p, DESTINATION as r, bookability as s, AMENITY_LABELS as t, disabledMatchers as u, nightLabel as v, useBookingStore as w, parseStaySearch as x, nightsBetween as y };
